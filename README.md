...
- 视频不设置 720p、1080p、2K 或 4K 上限，自动选择平台返回的最高质量
...
| 抖音 | ⚠️ 实验性 | — | ✅ | 视频；主页由 Playwright 滚动发现，受空白页、验证码和动态 DOM 影响较大 |
...
小红书与抖音的主页并不是 `yt-dlp` 2026.08.19 原生支持的 playlist URL：本项目使用 Playwright 加载主页并滚动发现每条作品，再逐条下载。网站页面和风控规则经常变化，因此主页发现比单条链接更容易需要 Cookie、验证码或后续适配。

## “最高质量”和“原图”的含义

本项目不会人为限制分辨率：有 4K 就选 4K，有 2K 就选 2K，否则继续选择平台返回的下一档最高质量。

- YouTube、Bilibili 和抖音使用 `bestvideo*+bestaudio/best`，不添加最大高度条件。
...
这里的“原图/原视频”是平台当前向该账号和网络环境提供的最高质量文件，不代表作者上传前、尚未经过平台处理的本地母版。会员等级、登录状态、地区、视频编码和平台接口返回结果都可能影响最高可用清晰度。
...
1. 点击“打开 Chrome 验证”，或使用普通 Chrome 打开任务的原始主页或视频链接。
...
以下结果来自 2026-08-26、本机 `yt-dlp 2026.08.19`，使用元数据模拟、主页发现或 Range 请求，没有整批下载测试主页：
...
| 抖音 | `https://www.douyin.com/user/MS4wLjABAAAAXTHbW60Q4K0cQ2WVWgYvE33ixusssYFX4n55a7lBjuQ` | 曾成功读取公开首屏，但后续出现空白页与加载超时，因此主页功能标记为实验性 |
| 抖音 | `https://www.douyin.com/video/7628957913016552758` | 本机 Cookie 不够新时进入 `needs_auth`；完成 Chrome 验证后可重试 |
...
工具没有设置分辨率上限。通常原因是平台本次只向当前账号返回到 1080p、账号没有对应权限、Cookie 失效、地区限制，或原作品本身没有更高版本。Bilibili 的部分 4K/HDR/高码率格式需要登录或会员权限。
