...
- 抖音不会只相信网页的“2K/4K”标签或 `yt-dlp` 的默认格式排序：工具从已经校验作者和作品 ID 的数据中取得媒体 ID，同时读取作者作品接口中与该作品绑定的最高档直连流，优先探测这些直连流和抖音高质量播放端点的 `default` 原始档，再探测 4K、2K、1080p 和 720p 档，并用 FFprobe 检查媒体流的实际像素、编码、码率和时长，然后选择实际质量最高的有效流。同一最高档直连的冗余镜像不要求全部成功；官方档位遇到 timeout、429 或临时 5xx 时会先做有限退避重试。重试后只要仍有一个官方档位无法确认，或已验证候选的最高实际像素仍低于作者接口给出的最高档，本次就会明确失败，不会趁机下载已经探测到的 720p/1080p 低档版本。
...
应用不会自动破解验证码。抖音作者接口也可能返回 HTTP 200 但暂时不给作品列表，这通常是独立于验证码的短时限流。程序会串行执行签名请求以避免多个任务同时冲击接口，切换请求页大小、在同一签名会话内退避，并用全新签名会话冷却重试；仍未恢复时会明确显示“短时限流”，保留重试按钮且不打开作者主页，也不会改下低清版本。遇到真正的登录或验证码错误时才显示 `等待验证`。频繁连续抓取仍可能再次触发风控。
...
旧版本在抖音把视频页异常包装成博主主页列表时，可能误把列表中的其他作品加入单视频任务；主页弹层形式的 `/user/...?...modal_id=<视频 ID>` 也可能被误判为主页。当前版本会把弹层链接规范化为唯一的 `/video/<视频 ID>`，并强制抖音单视频任务只能包含这一个经过 ID、来源域名和媒体身份校验的作品；Chrome 验证也始终使用这个原视频地址。更新并彻底重启后，保留原视频地址的历史任务会在启动时立即清除未下载的数字队列，点击继续后从原视频重新解析；已有文件不会删除。如果旧状态已经把原视频地址丢成了博主主页，程序会隔离该任务、禁用验证和重试，不会猜测目标 ID 或继续打开错误主页；请重新粘贴原始 `/video/<视频 ID>` 创建任务。
