...
- 抖音不会只相信网页的“2K/4K”标签或 `yt-dlp` 的默认格式排序：工具从已经校验作者和作品 ID 的数据中取得媒体 ID，同时读取作者作品接口中与该作品绑定的最高档直连流，优先探测这些直连流和抖音高质量播放端点的 `default` 原始档，再探测 4K、2K、1080p 和 720p 档，并用 FFprobe 检查媒体流的实际像素、编码、码率和时长，然后选择实际质量最高的有效流。同一最高档直连的冗余镜像不要求全部成功；但如果五个官方档位中任何一个无法确认，或已验证候选的最高实际像素仍低于作者接口给出的最高档，本次会明确失败，不会趁机下载已经探测到的 720p/1080p 低档版本。
...
### 为什么粘贴一个抖音视频，却出现很多纯数字作品？

旧版本在抖音把视频页异常包装成博主主页列表时，可能误把列表中的其他作品加入单视频任务；主页弹层形式的 `/user/...?...modal_id=<视频 ID>` 也可能被误判为主页。当前版本会把弹层链接规范化为唯一的 `/video/<视频 ID>`，并强制抖音单视频任务只能包含这一个经过 ID、来源域名和媒体身份校验的作品；Chrome 验证也始终使用这个原视频地址。发现旧的异常任务时，先取消正在运行的任务，更新并重启程序，再点击重试；错误条目会被移出任务，已经存在的文件不会被删除。如果历史状态已经丢失原视频地址，重试按钮会被禁用，请用原始 `/video/<视频 ID>` 新建任务。
