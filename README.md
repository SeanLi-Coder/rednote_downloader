...
- 抖音不会只相信网页的“2K/4K”标签或 `yt-dlp` 的默认格式排序：工具从已经校验作者和作品 ID 的数据中取得媒体 ID，同时读取作者作品接口中与该作品绑定的最高档直连流，探测这些直连流以及 `default`、4K、2K、1080p 和 720p 官方播放档，并用 FFprobe 检查媒体流的实际像素、编码、码率和时长，然后选择实际质量最高的有效流。普通视频会直接逐字节下载刚刚验证过的同一条最高画质地址，不允许 `yt-dlp` 在落盘阶段重新选择格式；完整传输后还会再次核对最终跳转域名、字节数、实际分辨率、码率和时长，全部一致才原子替换目标文件。Live Photo 使用同样的完整档位探测；真实案例中 `default` 比页面元数据直连高出一个分辨率档。官方档位遇到 timeout、429 或临时 5xx 时会先做有限退避重试。重试后只要仍有一个官方档位无法确认，或已验证候选的最高实际像素仍低于作者接口给出的最高档，本次就会明确失败，不会趁机下载已经探测到的 720p/1080p 低档版本。
...
以下结果来自 2026-08-26 至 2026-08-27、本机 `yt-dlp 2026.08.19`。抖音 148 条测试主页做了整批下载、失败重试和落盘复核；其余清晰度测试使用有限 Range 探测或注明的单条样本下载：
...
| 抖音 | 上述主页中分散抽样的 5 个 Live Photo | 5/5 的 `default` 实测高于元数据直连：720×1280/720×960 提升到 1080×1920、1308×1744 或 1080×1440；其余 4K/2K/1080p/720p 档反而返回低档，因此程序会逐档 FFprobe 后选择 `default` |
