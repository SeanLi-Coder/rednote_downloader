...
旧程序不会因为 `git pull` 自动退出。旧进程仍占用 `127.0.0.1:8765` 时，再次运行启动脚本会拒绝加载第二个任务管理器，也不会把 Chrome 打开到旧进程；Terminal 会明确提示端口被旧版本占用。请回到之前启动下载器的 Terminal 按 `Ctrl+C`，再重新运行 `./start.command`。如果找不到旧窗口，可用下面的只读命令查看占用端口的进程：
